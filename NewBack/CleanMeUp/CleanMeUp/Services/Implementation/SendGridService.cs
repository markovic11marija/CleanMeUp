...
namespace CleanMeUp.Services.Implementation
{
    public class SendGridService : ISengridService
    {
        ...
        public async Task<bool> SendMailAsync(Order model)
        {
            ...
            var htmlContent = $"<div><p><b>Detalji zahteva</b></p></div><body><div><p>Adresa dostavu: {model.DeliveryAddress }</p><p>Adresa za preuzimanje: {model.PickUpAddress}</p><p>Telefon: {model.Phone }</p><p>Email: {model.Email }</p><p>Datum: { DateTime.Now: dd/MM/yyyy HH:mm:ss}</p><p>ReferencaBanke: {model.BankReferenceId}</p><p>Stavke: </p>{model.ReturnItems()}</div></body>";
